/**
 * CS生産性タブ 自動集計スクリプト
 * 対象: 田中春奈、平松弥央菜、小林陽香、小林未侑、中田菜々子、久保梨生、宇梶知恵、増子真也子、田中里奈、山下優花、中村八重子、松元陸、佐藤大河、西田真優
 *
 * A列: 年月（月の先頭行のみ）
 * B列: メンバー名（全行に記載）
...
var HELPER_SHEET_NAME = '_カテゴリマスタ';
...
function updateCSProductivity() {
  ...
  if (dropdownList.length > 0) {
    // ヘルパーシートにカテゴリ一覧を書き出し（requireValueInListの文字数上限を回避）
    var helperSheet = targetSS.getSheetByName(HELPER_SHEET_NAME);
    if (!helperSheet) {
      helperSheet = targetSS.insertSheet(HELPER_SHEET_NAME);
      helperSheet.hideSheet();
    } else {
      helperSheet.clear();
    }

    var catValues = dropdownList.map(function(cat) { return [cat]; });
    helperSheet.getRange(1, 1, catValues.length, 1).setValues(catValues);

    var catRange = helperSheet.getRange(1, 1, catValues.length, 1);
...
function parseSheetDate(sheetName) {
  ...
  // パターン2b: 3桁数字（例: 215 → 2月15日、110 → 1月10日）
  ...
  // パターン4: 「YYYY/M/D」または「YYYY-M-D」形式（例: 2026/1/1、2025-12-24）
